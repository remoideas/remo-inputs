(function ($) {
	...
	$.fn.repeatable = function (userSettings) {
		...
		var defaults = {
			...
			fadeContainer: '.main-repeat-element',
			fadeTrigger: ".title-repeat-element",
			onAdd: function () {},
			onDelete: function () {}
		};
		...
		/**
		 * Hide/show elemento main content of each group. 
		 */
		
		var fadeOne = function () {
			$(this).parent().parent().find(settings.fadeContainer).fadeToggle('fast');
		};
		...
		*/
		...
		(function () {
			...
			$("form").on("click", settings.fadeTrigger, fadeOne);
